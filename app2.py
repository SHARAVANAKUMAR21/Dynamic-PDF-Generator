...
# Function to get the next Invoice ID
def get_next_invoice_id():
    # File to store the last used Invoice ID
    invoice_id_file = "invoice_id.txt"
    
    # Check if the file exists
    if os.path.exists(invoice_id_file):
        # Read the last used Invoice ID
        with open(invoice_id_file, "r") as file:
    ...
    # Save the new Invoice ID back to the file
    with open(invoice_id_file, "w") as file:
        file.write(str(next_id))
    
    return next_id

# Function to format the document (assumed edit function)
def edit_word_template(template_path, word_output_path, placeholders, font_name, font_size, option):
    # Assuming the function replaces placeholders with actual data, 
    # including <<Invoice ID>> with the generated Invoice ID
...
# If Invoice is selected, generate Invoice ID
if option == "Invoice":
    invoice_id = get_next_invoice_id()  # Get the next Invoice ID
    placeholders = {
        "<<Invoice ID>>": str(invoice_id)  # Add the Invoice ID placeholder to be replaced
    }
    # Proceed with the rest of the invoice generation logic here...
    # Call generate_invoice() or other functions as needed
    generate_invoice()  # Assuming this function handles invoice generation

# For other document types
if option not in ["Invoice"]:
